...
import { requireBroadcaster } from './utils/broadcasterVerifier.js'
...
app.post('/register-deposit', async (req, res) => {
  ...
  try {
    ...
    console.log(`📥 Manual deposit registration for ${address}: ${amount}`)
    
    // Import openChannelForUser
    const { openChannelForUser } = await import('./sessionManager.js')
    
    // Create Yellow channel
    await openChannelForUser(address, amount.toString())
    
    console.log(`✅ Yellow channel created for ${address}`)
    
    res.json({
      success: true,
      message: 'Deposit registered and Yellow channel created',
      address,
      amount,
      txHash
    })
    ...
    console.error('Register deposit error:', error)
    res.status(500).json({
      success: false,
      error: error.message
    })
...
app.post('/broadcast-trade', async (req, res) => {
  ...
  try {
    ...
    if (!broadcasterAddress || !direction) {
      return res.status(400).json({ 
        success: false, 
        error: 'Missing broadcasterAddress or direction' 
      })
    }
    ...
    if (!['BUY_ETH', 'SELL_ETH'].includes(direction)) {
      return res.status(400).json({
        success: false,
        error: 'Invalid direction. Must be "BUY_ETH" or "SELL_ETH"'
      })
    }

    // ✅ VERIFY BROADCASTER IS REGISTERED ON-CHAIN
    console.log(`🔍 Verifying broadcaster registration: ${broadcasterAddress}`)
    const broadcasterInfo = await requireBroadcaster(broadcasterAddress)
    console.log(`✅ Broadcaster verified:`, broadcasterInfo.data?.name || 'unnamed')

    const currentEthPrice = ethPrice || 3000
    
    // Determine trade type and tokens from direction
    const trade = {
      type: direction === 'BUY_ETH' ? 'buy' : 'sell',
      direction: direction,
      fromToken: direction === 'BUY_ETH' ? 'pyusd' : 'eth',
      toToken: direction === 'BUY_ETH' ? 'eth' : 'pyusd'
    }
    ...
    res.json({
      ...result,
      broadcasterAddress,
      broadcasterInfo: broadcasterInfo.data,
      direction,
